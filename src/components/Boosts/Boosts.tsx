      ...
      {boosts?.length > 0 && (
        <div className="my-5 px-3 flex justify-start gap-6">
          <div className="flex flex-col justify-center w-30 h-30 overflow-hidden rounded-3xl">
            <img src={boosts[0].boost_photo_url} alt={boosts[0].name} />
          </div>
          <div className="flex flex-col gap-1">
            <h3 className={`${s.font} text-zinc-200 tracking-wider`}>
              {boosts[0].name}
            </h3>
            <p className={`${s.font} text-zinc-500 text-sm`}>
              {boosts[0].desc}
            </p>
            <div className="flex items-center gap-1">
              <p className={`${s.font} text-zinc-200 text-sm`}>
                {boosts[0].price}
              </p>
              <img
            ...
            <button
              ...
              onClick={() =>
                handeBuyInPhenerium(
                  Number(boosts[0].price),
                  boosts[0].idItem,
                  boosts[0].collectionId,
                )
              }
            >
              {IsLoading ? (
